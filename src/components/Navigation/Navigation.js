    ...
    return (
      <div className="navbar">
        ...
        <nav className="navbar__links">
          <NavLink className={`navbar__link navbar__link` + navClass}>Home</NavLink>
          <NavLink className={`navbar__link navbar__link` + navClass}>Saved articles</NavLink>
          { isLoggedIn ? (
            <button className={`navbar__header-button navbar__header-button_logout navbar__header-button` + navClass}>
              { userName }
              <img src={ location.pathname === '/' ? logout_white : logout_black } alt='logout icon'/>
            </button>
          ) : (
            <button className={`navbar__header-button navbar__header-button_login navbar__header-button` + navClass}>Sign in</button>
          )}
        </nav>
      </div>
    )
